...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.coffiebot/workspace"
...
class OpenVikingConfig(Base):
    """OpenViking 上下文记忆数据库配置。"""

    enabled: bool = False
    server_url: str = ""  # OpenViking 服务地址，如 http://your-server:1933
    api_key: str = ""  # OpenViking API Key
    agent_id: str = "coffiebot"  # OpenViking agent（account）标识
    user_id: str = "coffiebot_user"  # OpenViking user 标识
    recall_limit: int = 5  # 语义检索返回的最大记忆条数
    recall_score_threshold: float = 0.0  # 语义检索最低相关性分数（0 表示不过滤，由 OV 分层检索自行排序）
...
class Config(BaseSettings):
    ...
    model_config = ConfigDict(env_prefix="COFFIEBOT_", env_nested_delimiter="__")
